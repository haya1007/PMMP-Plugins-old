...
class Main extends PluginBase implements Listener{
	public function onEnable(){
		...
		$this->getLogger()->notice("ThunderSword追加システム稼働");
		...
		$sword = Item::get(283);
		if($this->config->get("Unbreakable") === "on"){
   			$sword->setUnbreakable();
   		}
		$sword->setCustomName($name);
		$sword->setNamedTagEntry(new StringTag("thunder", "on"));

		$this->sword = $sword;
		Item::addCreativeItem($this->sword);
	}
	...
	public function onDamage(EntityDamageEvent $event){
		if($event instanceof EntityDamageByEntityEvent){
			...
			if($damager instanceof Player){
				...
				if($hand->getNamedTagEntry("thunder") !== null){
					$atk = $this->config->get('ATK');
					...
					$pk->yaw = 0;
					$pk->pitch = 0;
					$pk->position = new Vector3($entity->x, $entity->y-3, $entity->z);
					$server = $this->getServer();
	...
	public function onTap(PlayerInteractEvent $event){
		if($this->config->get("BlockTap") === "on"){
			if($event->getAction() === 0 || $event->getAction() === 1){
				if($event->getPlayer()->getInventory()->getItemInHand()->getNamedTagEntry("thunder") !== null){
					...
					$pk->yaw = 0;
					$pk->pitch = 0;
					$pk->position = new Vector3($block->getX(), $block->getY()-3, $block->getZ());
					$server = $this->getServer();
					$server->broadcastPacket($server->getOnlinePlayers(), $pk);
				}
			}
