...
class main extends PluginBase implements Listener{
	public function onEnable(){
		...
		$this->getLogger()->notice("LevelShopSystem稼働確認");
		if(!file_exists($this->getDataFolder())){mkdir($this->getDataFolder(), 0744, true);}
		$this->b = new Config($this->getDataFolder()."shop.yml",Config::YAML);
		if($this->getServer()->getPluginManager()->getPlugin("LevelMoneySystem") != null){
			$this->E = $this->getServer()->getPluginManager()->getPlugin("LevelMoneySystem");
			$this->getLogger()->info("LevelMoneySystemを検出しました。");
		}else{
			$this->getLogger()->warning("LevelMoneySystemが見つかりませんでした");
